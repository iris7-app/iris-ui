/**
 * about-data.ts — pure data extracted from about.component.ts so the
 * component file stays focused on the tab dispatcher logic.
 *
 * 5 arrays + 2 strings (~570 LOC of data) lived inline in the component
 * since the original About-page write-up. Moving them here:
 * - shrinks about.component.ts from 652 → ~80 LOC (-87 %)
 * - keeps grep/blame on the data clean (one file owns it)
 * - makes the future per-tab widget extraction trivial : each widget
 *   imports only the data slice it needs (no parent prop drilling)
 *
 * Phase B-7-5 Phase 1A — extract data, no widget refactor yet.
 * Per ~/.claude/CLAUDE.md "1 widget = 1 fichier" rule, the per-tab
 * widget split (Phase 1B) follows in a separate batch.
 */
...
export const PORT_CATEGORIES = ['App', 'Data', 'Admin', 'CI', 'Obs', 'Infra'] as const;
...
export const TECHNOLOGIES: Technology[] = [
  ...
  },
  {
    ...
    description: 'Convention-over-configuration framework for production-ready Java applications.',
    usage:
...
export const PORT_MAP: PortMapEntry[] = [
  {
    port: 8080,
...
export const RUN_COMMANDS: RunCommand[] = [
...
export const QUICK_START = `# Start everything (backend + frontend)
./run.sh all
